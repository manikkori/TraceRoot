...
export function Dashboard() {
  ...
  // 3. Auth Protection Logic (Bina kisi gande loader ke)
  if (!isLoaded) {
    return null; // Jab tak auth check ho raha hai, blank rahega (no ugly spinner)
  }

  if (!isSignedIn) {
    return <RedirectToSignIn />; // Logged in nahi hai toh bahar phek dega
  }
          ...
          <div>
            <h1 className="text-2xl font-bold text-devText flex items-center gap-2">
              <img
                src="/image.png"
                alt="Logo"
                className="w-8 h-8 bg-white p-1 rounded-md object-contain"
              />
              RepoRescue AI
            </h1>
